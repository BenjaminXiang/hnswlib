...
// #include <ivf/imi.h>
// #include <ivf/l2_inverted_list.h>
...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    HierarchicalNSW(
        ...
        mult_ = 1 / log(1.0 * M_);
        revSize_ = 1.0 / mult_;
        length_ = (float *) malloc(sizeof(float) * max_elements_);
    }
...
//                    if (candidate_id == 0) continue;
    ...
    template <bool has_deletions, bool collect_metrics = false>
    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
    searchBaseLayerSQ8(tableint ep_id, const void *data_point, size_t ef, std::size_t &cal_cnt, BaseFilterFunctor* isIdAllowed = nullptr) const {
        VisitedList *vl = visited_list_pool_->getFreeVisitedList();
        vl_type *visited_array = vl->mass;
        vl_type visited_array_tag = vl->curV;
        auto computer = quant.get_computer((const float *)data_point);
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> candidate_set;

        dist_t lowerBound;
        if ((!has_deletions || !isMarkedDeleted(ep_id)) && ((!isIdAllowed) || (*isIdAllowed)(getExternalLabel(ep_id)))) {
            dist_t dist = fabs(computer(getExternalLabel(ep_id))); // fstdistfunc_(data_point, getDataByInternalId(ep_id), dist_func_param_);
            // printf("%0.5f\n", dist);
            ++cal_cnt;
            lowerBound = dist;
            top_candidates.emplace(dist, ep_id);
        ...
        while (!candidate_set.empty()) {
            ...
            tableint current_node_id = current_node_pair.second;
            const std::pair<labeltype, tableint> *data = edges.data() + current_node_id * (maxM0_ + 1);
            // int *data = (int *) get_linklist0(current_node_id);
            size_t size = data[0].second;
            if (collect_metrics) {
                metric_hops++;
                metric_distance_computations+=size;
            }
            ...
            for (size_t j = 1; j <= size; j++) {
                int candidate_id = (data + j)->first;
                if (!(visited_array[candidate_id] == visited_array_tag)) {
                    visited_array[candidate_id] = visited_array_tag;

                    // char *currObj1 = (getDataByInternalId(candidate_id));
                    dist_t dist = fabs(computer((data + j)->second));// fstdistfunc_(data_point, currObj1, dist_func_param_);
                    // printf("%d\n", dist);
                    ++cal_cnt;
...
//     void
//     searchWithIVF(const void *query, size_t k, imi::InvertedList &ivf, utils::ResultPool<float> &result) const {
//         std::size_t ef = std::max(ef_, k);
//         std::priority_queue<utils::Node<float>, std::vector<utils::Node<float>>, utils::MinHeap<float>> min_candidate;
//         std::priority_queue<utils::Node<float>, std::vector<utils::Node<float>>, utils::MaxHeap<float>> max_candidate;
...
//        output.close();
    ...
    */
    void addPoint(const void *data_point, labeltype label, bool replace_deleted = false) {
        ...
        length_[label] = fstdistfunc_(data_point, data_point, dist_func_param_);
        // std::cout << length_[label] << '\n';
        // lock all operations with element by label
        std::unique_lock <std::mutex> lock_label(getLabelOpMutex(label));
        if (!replace_deleted) {
    ...
    tableint addPoint(const void *data_point, labeltype label, int level) {
        ...
        if ((signed)currObj != -1) {
            ...
            for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                ...
                if (epDeleted) {
                    dist_t d = fstdistfunc_(data_point, getDataByInternalId(enterpoint_copy), dist_func_param_);
                    d = length_[label] * length_[getExternalLabel(enterpoint_copy)] - d * d;
    ...
    std::priority_queue<std::pair<dist_t, labeltype>>
    searchKnnCntSQ8(const void *query_data, size_t k, std::size_t &cal_cnt, BaseFilterFunctor* isIdAllowed = nullptr) const {
        std::priority_queue<std::pair<dist_t, labeltype >> result;
        if (cur_element_count == 0) return result;
        ...
        if (num_deleted_) {
            ...
            top_candidates = searchBaseLayerSQ8<false, true>(
                    currObj, query_data, std::max(ef_, k), cal_cnt, isIdAllowed);
        }
